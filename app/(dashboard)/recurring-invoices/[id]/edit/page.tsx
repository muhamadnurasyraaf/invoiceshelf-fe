...
const statusOptions: { value: RecurringInvoiceStatus; label: string }[] = [
...
export default function EditRecurringInvoicePage() {
  ...
  const loadData = async () => {
    try {
      setIsLoading(true);
      const [recurringData, customersData, itemsData] = await Promise.all([
        recurringInvoiceService.getById(recurringId),
      ...
      reset({
        ...
        endDate: recurringData.endDate
          ? new Date(recurringData.endDate).toISOString().split("T")[0]
          : "",
        dayOfMonth: recurringData.dayOfMonth || 1,
        dayOfWeek: recurringData.dayOfWeek || 1,
        dueAfterDays: recurringData.dueAfterDays,
        notes: recurringData.notes || "",
        items: recurringData.items.map((item) => ({
          itemId: item.itemId,
          quantity: item.quantity,
        })),
      });
    } catch {
      setError("Failed to load recurring invoice");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const onSubmit = async (data: RecurringInvoiceFormData) => {
    ...
    setError(null);
    try {
      await recurringInvoiceService.update(recurringId, {
        ...data,
        endDate: data.endDate || undefined,
      });
      router.push("/recurring-invoices");
    } catch {
      setError("Failed to update recurring invoice. Please try again.");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-500 mb-4">
          The recurring invoice you&apos;re looking for doesn&apos;t exist.
        </p>
        <Link
        ...
        {recurring && (
          <p className="text-sm text-gray-500 mt-1">
            ...
            {recurring.lastGeneratedAt &&
              ` - Last: ${new Date(recurring.lastGeneratedAt).toLocaleDateString()}`}
          </p>
        )}
      </div>
